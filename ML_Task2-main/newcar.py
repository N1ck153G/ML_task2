...
class Car:
    ...
    """ 2. This Function:
    This function is responsible for rendering a rotated sprite's specific location on the screen, and also drawing the sensor lines.  
    """
    ...
    """ 3. This Function:
    The 'draw_rader' function is a method that loops through of radar/sensor positions ('self.radars') and always draws line on the screen to visualise it.
    The lines connect from the center point of the car. 
    """
    ...
    """ 4. This Function:
    The 'check_collision' function is a method that checks wether any of the conners have touched the border of the track, with the object being represented by 'self'.
    If the object 'self' has collided with the boundry represented by 'border_colour' on the game map, 'self.alive' is set to false.
    Indicating that the object is no longer alive due to the collision and stoping the loop via the use of the command 'break'.
    """
    ...
    """ 5. This Function:
    'check radar' is a function that simulates a radar that starts from a specific angle of the object (car) extending all the way out untill it hits the border of the track, which is defined by 'border_color'.
    Or reaches a maximum lenght of 300 units. When the sensor hits a border it records the length, saving the data to list for later use in obstacle avoidence.
    ...
    """ 6. This Function:
    This function checks various parts of the object including updating it's position, calculating distance and time, checking for collisions, and preforming radar and sensor checks in multiple directions,

    first it checks if 'self.speed_set' is 'false'
    if it is set to false 'self.speed' is set to 20 and 'self.speed_set' is set to true

    Sprite rotation:
    rotates the sprite based of 'self.angle' attributes and updates the visuals
    updates 'self.position' in the in the X direction based of speed and angle
    ensures that object dosen't get to close to the edges of the of the game screen

    Distance and Time Tracking:
    Updates 'self.distance'
    adds '1' to 'self.time'

    Center and Corners Calculation:
    Calculates the new center position (self.center) of the object.
    Calculates the coordinates of the four corners of the object's bounding box (self.corners) based on its position and angle.

    Collision Detection:
    Calls the check_collision method to check for collisions between the object and the game map boundaries.
    updates the self.alive attribute based on collision detection.

    Clear Radar Data:
    Clears the self.radars list, in preparation for radar or sensor checks in the following loop.

    Radar/Sensor Checks:
    Iterates over a range of angles (-90 to 120 with a step size of 45) to perform radar or sensor checks in multiple directions.
    Calls the self.check_radar method for each angle to collect radar or sensor data.
    ...
    """ 7. This Function:
    the 'get_data' method collects data from radar or sensor readings stored in self.radars. It processes the distances by dividing them by 30 and returning the processed data in the form of a list with five elements.
    """
    ...
    """ 8. This Function:
    is_alive method is used to determine if the object is currently alive or not. It returns the value of the 'self.alive'
    """
    ...
    """ 9. This Function:
    The 'get_reward' function is a method within a Python class, and its purpose is to calculate and return a reward value. 
    """
    ...
    """ 10. This Function:
    rotate_center function takes an image and an angle as input, rotates the image around its center by the given angle, and returns the rotated image with the same dimensions as the original image.
    """
